...
describe('organic shoreline junctions', () => {
  for (const inward of [false, true]) for (let rotation = 0; rotation < 4; rotation++) {
    it(`${inward ? 'inward' : 'outward'} corner ${rotation} connects across every edge`, () => {
      ...
      for (let y = 1; y < 5; y++) for (let x = 1; x < 5; x++) {
        ...
        for (let i = 0; i <= 32; i++) {
          const t = i / 32
          expect(shorelineCoverage(mask, 1, t)).toBeCloseTo(shorelineCoverage(maskAt(x + 1, y), 0, t), 10)
          expect(shorelineCoverage(mask, t, 1)).toBeCloseTo(shorelineCoverage(maskAt(x, y + 1), t, 0), 10)
          const alpha = shorelineCoverage(mask, t, t)
