...
export class MapRenderer {
  ...
  drawOreOverlay(ctx, tileX, tileY, screenX, screenY, useTexture, density = 1) {
    ...
    if (integratedTile?.rect && integratedTile?.image) {
      const underlayTile = this.textureManager.selectCrystalBiomeUnderlayTile(tileX, tileY)
      if (underlayTile?.rect && underlayTile?.image) {
        this.drawIntegratedTileImage(ctx, underlayTile, screenX, screenY)
      }
      this.drawIntegratedTileImage(ctx, integratedTile, screenX, screenY)
      return
    }
