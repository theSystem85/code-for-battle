...
export const updateAmmunitionTruckLogic = logPerformance(function(units, gameState, delta) {
  ...
  ammoTrucks.forEach(ammoTruck => {
    ...
    if (ammoTruck.ammoResupplyTarget) {
      ...
      if (target.health <= 0 ||
          (isUnit && (target.type === 'apache' || target.type === 'f35') && typeof target.maxRocketAmmo === 'number' && target.rocketAmmo >= target.maxRocketAmmo) ||
          (isUnit && target.type !== 'apache' && target.type !== 'f35' && typeof target.maxAmmunition === 'number' && target.ammunition >= target.maxAmmunition) ||
          ...
          if (ammoTruck.ammoCargo > 0) {
            ...
            if (needsAmmo) {
              const refillRate = (isUnit ?
                ((target.type === 'apache' || target.type === 'f35') ? target.maxRocketAmmo : target.maxAmmunition) :
