...
- Every aircraft assigned to a carrier deck renders in the upper unit layer after the carrier hull, including grounded parked and taxiing aircraft, so the carrier can never paint over a landed F22, F35, or Apache.
...
- A hull-issued target makes the battleship turn the nearer side toward that target before firing. Each mount refuses shots whose line crosses the central control-tower exclusion cone. Battleship selection renders no per-turret green dashed firing circles or red blocked-angle wedges; only the compact selected-turret ring remains.
...
- Submarines can attack naval units, enemy partly-water Shipyards, and enemy Construction Yards as an explicit command-center exception, but no other wholly land-based unit or building.
...
- Under the focused six-times CPU-throttled browser benchmark, one selected layered battleship plus a carrier with four parked F22s must retain at least 80% of the same entity scene's ground-layer baseline FPS; the benchmark also records update/render/entity timing and JavaScript heap movement. The command is `PERF_BATTLESHIP_RENDER=1 PLAYWRIGHT_SKIP_WEB_SERVER=1 PLAYWRIGHT_BASE_URL=http://127.0.0.1:5173 npx playwright test tests/e2e/battleshipPerformance.test.js --project=chromium --reporter=line`.
