...
17. Even when `Custom sprite sheets` is disabled, ore + seed crystal rendering must still resolve tagged tiles from bundled `crystals_q90_1024x1024` metadata/image as the default integrated crystal source; once custom sprite sheets are enabled, custom crystal-tagged candidates must override the bundled default when matching tags exist.
18. Crystal rendering must never visually stack a new integrated crystal sprite on top of the legacy per-file ore/seed tile in the same frame; only one crystal resource layer may render while still keeping the underlying terrain tile (grass/soil/snow/street) visible as the background.
