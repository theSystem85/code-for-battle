...
- **FR-057**: System MUST treat placed `street` and `concreteWall` tiles as non-expanding anchors for general building proximity; they may only extend placement range for additional buildings of that same type, preventing roads/walls from unlocking unrelated building placements farther from the base.
- **FR-058**: System MUST apply the same street-only and wall-only proximity extension rule to queued blueprints/plans during validation and production start, so planned road/wall chains can keep building outward while unrelated building types cannot use those chains as anchors.
