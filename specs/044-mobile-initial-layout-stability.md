...
- Safe-area defaults must not depend on `body.is-touch` for first paint; base `body` custom properties should resolve directly from `env(safe-area-inset-*)` with `0px` fallback.
- Runtime safe-area sync should execute even before touch detection settles, and include an extra delayed pass to catch iOS asynchronous inset initialization on cold PWA launch.
