...
- `tank-v2` alert mode must render only the larger alert-range ring (`TANK_FIRE_RANGE`) and must not render the inner tile circle.
- Utility service units in alert mode keep their existing dual-ring visualization (tile circle + larger discovery-range ring).
