...
10. The sidebar speed control must also persist to localStorage (`rts-game-speed-multiplier`) and be restored during normal startup (without requiring a save/autosave load), so browser refresh preserves the chosen speed.
11. Movement/pathfinding support timers that influence unit steering or stuck recovery must stay on simulation time, and attack-path `moveTarget` values must remain in tile coordinates so units do not oscillate between adjacent tiles under increased speed multipliers.
12. Altitude-adjusted airborne hit tests must use the same lifted render-space target center as air-target aiming, so F22 and Apache rockets still apply direct damage to airborne aircraft under the fixed-step simulation.
13. Transient overlay effects that are stamped with simulation timestamps, including Tesla lightning and smoke particles emitted by buildings, projectiles, and damaged units, must age and expire on simulation time rather than raw wall-clock time.
14. Camera scrolling, right-drag inertia, keyboard panning, and smooth minimap scroll interpolation must stay tied to render/UI frames and wall-clock input cadence, and must not speed up or decay faster when `gameState.speedMultiplier` increases.
