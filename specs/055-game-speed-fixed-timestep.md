...
11. Altitude-adjusted airborne hit tests must use the same lifted render-space target center as air-target aiming, so F22 and Apache rockets still apply direct damage to airborne aircraft under the fixed-step simulation.
12. Transient overlay effects that are stamped with simulation timestamps, including Tesla lightning and smoke particles emitted by buildings, projectiles, and damaged units, must age and expire on simulation time rather than raw wall-clock time.
13. Camera scrolling, right-drag inertia, keyboard panning, and smooth minimap scroll interpolation must stay tied to render/UI frames and wall-clock input cadence, and must not speed up or decay faster when `gameState.speedMultiplier` increases.
