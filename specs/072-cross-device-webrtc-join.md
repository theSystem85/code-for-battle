...
- Netlify's available API does not expose function logs. Offers were not logged with the player alias, so the failed joins for "galina" and "max" could not be reconstructed from retained logs.
...
2. The response always includes Google STUN. When `TURN_URLS` is set, it also includes TURN:
   - `TURN_SECRET` mints a 12-hour coturn `static-auth-secret` username (`<unix-expiry>:cfb`) and base64 HMAC-SHA1 credential.
   - Otherwise `TURN_USERNAME` and `TURN_CREDENTIAL` are passed through.
   - `VITE_TURN_URLS`, `VITE_TURN_USERNAME`, and `VITE_TURN_CREDENTIAL` are a build-time fallback used only when that request fails.
...
8. Logs use alias, candidate type, protocol, mDNS, and private/loopback flags. They do not include IP addresses, the TURN secret, usernames, or credentials. The join UI shows a TURN hint when the attempt fails with no relay candidates.
9. `/api/*` and `/.netlify/functions/*` bypass the service worker cache. The cache name is `code-for-battle-cache-v2` so previously cached API responses are dropped.

## TURN setup
Set these on the Netlify site for Functions, including production and deploy previews:
...
2. Mac host, iPhone client, iPad client, all on the same Wi-Fi. Each phone submits its alias from the invite link. The host notification names the alias, and both phones leave "Connecting to host...".
3. Repeat with one client on cellular (Wi-Fi off). The join should still complete, and Netlify function logs for `signalling` `candidate` events from that alias should include `type: "relay"`.
4. Without TURN variables, a same-computer two-browser join still works. A phone join should fail with the on-screen hint that no TURN server is configured, and the host log should show `relay: 0`.
