...
10. `/api/*` and `/.netlify/functions/*` bypass the service worker cache. The cache name is `code-for-battle-cache-v3` so previously cached API responses are dropped. Navigations other than `/` and `/index.html` are not written over the cached app shell.
