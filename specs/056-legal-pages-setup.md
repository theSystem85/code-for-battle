...
- 2026-04-13: Localize the Impressum/Imprint contact-form links so the German legal notice points to `/kontakt` and the English imprint points to `/contact` even when the stored config contains only one contact-form path.
- 2026-04-13: Add explicit hidden `form-name` fields to both Netlify contact forms. This makes the submitted payload self-describing and hardens custom success-page redirects on deploy previews where automatic form metadata injection may not be sufficient.
- 2026-04-13: Preserve the sidebar legal links as a no-JavaScript fallback on portrait mobile first paint by gating the CSS-only sidebar-hiding fallback behind a `html:not(.no-js)` selector. Crawlers and LLM/browser checks that do not execute runtime layout JavaScript still receive visible, clickable legal anchors.
