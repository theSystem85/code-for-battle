...
- 2026-04-16: Destroyed ground units now freeze in-place for 2 seconds once HP reaches 0, then trigger the destruction explosion + wreck registration; the destruction explosion SFX now plays at this delayed moment so audio aligns with the visual blast.
...
- **FR-059**: Defensive turret guns MUST not fire when a building blocks the line-of-sight to their target.
...
- Enemy ground units that are not rocket-capable anti-air (e.g., `tank`, `tank_v1`, `tank-v2`, `tank-v3`, `howitzer`) must not retain, reacquire, or path toward airborne `apache`/`f22Raptor`/`f35` targets.
- Rocket-capable anti-air units (`rocketTank`) and air units (`apache`, `f22Raptor`) may still target airborne `apache`/`f22Raptor`/`f35` targets.
