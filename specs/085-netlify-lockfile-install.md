...
The unfinished timer is `idealTree:node_modules/vitest` while optional peers pull both vitest 4.x and vitest 5.x (`vitest@*`). `npm ci` against the committed lockfile succeeds. This failed Deploy Previews that still used the old command, including PR 685 (`cursor/loading-screen-f29f`) during `building site` with exit code 2. The same install command is already on `main`.
