...
- On a phone in portrait (`orientation: portrait` and `max-width: 600px`), the settings dialog and the other game dialogs fill the dynamic viewport (`100vh`, then `100dvh`). They have no corner radius, margin, or max-width/max-height cap. `env(safe-area-inset-*)` insets the dialog so the header and close control stay clear of the notch and home indicator. The modal body is the only scroller; the header stays put. Desktop, tablet, and landscape layouts are unchanged. The global slim scrollbar (`scrollbar-width: thin` and the transparent-track `::-webkit-scrollbar` rules) stays in place.
