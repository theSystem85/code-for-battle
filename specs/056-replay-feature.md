...
- That terminal-marker halt cannot discard already-due deferred replay entries. If classic-AI or LLM commands from the final recorded tick have already been queued into the post-tick replay phase, replay must flush them before final pause; otherwise late building placements, rally-point updates, and party budgets diverge even without the extra simulation step.
- Deterministic replay/save comparisons also require stable runtime entity identifiers. Any ids generated after the replay baseline must avoid wall-clock sources such as `Date.now()` during deterministic sessions, otherwise identical post-baseline buildings/units can receive different ids and scramble save comparison alignment. Comparison tooling should sort buildings and rally-point collections by stable gameplay fields (owner/type/position) before using `id` as a tiebreaker.
...
- Guard-mode replay capture must include both direct single-target guard commands and AGF drag-assigned friendly guard groups (`guardTargetRefs`), so playback restores `guardMode`, `guardTarget`, and ordered `guardTargets` lists exactly as recorded.
- Unit entity references inside replay commands (including guard targets) must carry deterministic fallback identity fields (`owner`, `type`, `replaySpawnOrdinal`, `buildDuration`) and resolve through replay alias maps so commands still bind when runtime ids differ from the recording.
- Replay `cheat_code` entries must store cursor world coordinates used at record time, and playback must apply those coordinates while executing the cheat so spawn/build cheats reproduce the same positions regardless of replay viewer camera state.
