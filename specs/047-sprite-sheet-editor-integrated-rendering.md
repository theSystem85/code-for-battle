...
- Animated mode requirements:
  ...
  - Integrated street tile rendering now defaults to include `streets23_q90_1024x1024.webp` in static sheet selectors, resolves `street` tiles using neighbor-direction tags (`top`, `bottom`, `left`, `right`) with highest-valid-match selection, prefers the active biome tag (`grass`/`soil`/`snow`/`sand`) when those variants exist, and suppresses SOT rendering on street-host tiles.
