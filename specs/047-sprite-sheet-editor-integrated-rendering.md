...
- Follow-up (2026-04-19): SSE static tagging includes a `group` paint mode that paints rectangular clusters as `group_<id>` labels (`id` range 1-999) with sidebar Group ID manual override + auto-increment on drag release. Runtime integrated rendering consumes grouped tags for `rocks`, `decorative`, and `debris` tiles, preferring largest-fitting rectangular grouped variants for rocks/decorative regions and exact footprint-size debris groups (falling back to 1x1 debris when no grouped footprint match exists).
