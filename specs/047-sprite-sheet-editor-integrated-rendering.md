...
- Animated mode requirements:
  ...
  - Integrated runtime tile selection must support combining multiple static sprite sheets at once: gather all checked sheets from map settings, load only sheets whose JSON metadata contains at least one tagged tile, merge same-tag candidate buckets across sheets (e.g. `rocks` from A and B), and choose deterministic per-tile variation from the merged pool.
  - Local-storage metadata (`rts-sse-metadata:<sheetPath>`) must override sidecar JSON metadata for static sheets in singleplayer, but multiplayer must ignore those local overrides and rely on default sidecar metadata only (cosmetic-only, no host-client sync).
  - Map Settings must show a checkbox list of all available static sprite sheets under `Custom sprite sheets` (list shown only while enabled, capped to ~5 visible rows with scrolling) so users can opt sheets in/out of runtime map rendering.
  - Custom `rock` tiles must support alpha/black blend metadata while always rendering over the currently selected biome land base, matching ore-style overlay composition and never affecting simulation/pathing logic.
