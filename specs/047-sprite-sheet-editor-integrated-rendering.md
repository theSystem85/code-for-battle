...
- Animated mode requirements:
  ...
  - Static SSE tagging now supports rectangular tile groups via a `group` tag workflow: painting with active `group` tag writes `group_X` labels (X from sidebar `Group ID`, range 1-999), drag painting for groups is rectangle-only, and releasing mouse auto-increments `Group ID` by one.
  - The sidebar must expose the `Group ID` numeric input directly below `Add tag`, and user-entered values must immediately control the next `group_X` assignment.
  - Runtime grouped placement must initially support `rocks`, `decorative`, and `debris`: when connected map regions permit, rendering should place the largest matching rectangular group first, then fill remaining cells with smaller groups/singles.
  - Debris rendering must prefer a grouped tile set that exactly matches destroyed-building footprint (`width x height`); if no matching grouped debris exists, fall back to standard single-tile debris candidates.
