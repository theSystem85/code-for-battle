- [x] SSE multi-sheet runtime composition (2026-04-17): integrated custom rendering now combines all checked sprite sheets that have tagged tiles, honors per-sheet localStorage metadata overrides over sidecar JSON, skips untagged sheets to reduce memory, supports per-sheet rock blending over biome land underlay, and exposes a max-5-row checked-sheet list beneath the `Custom sprite sheets` toggle.
