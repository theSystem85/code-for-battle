- [x] Crystal biome underlay restoration beneath ore/seed overlays (2026-04-20): when bundled/default crystal sprites render (including with `Custom sprite sheets` disabled), ore and seed overlays now first paint deterministic biome-matched passable underlay tiles so transparent crystal pixels reveal the same random selected-biome ground texture as surrounding terrain.
- [x] Crystal sheet default runtime fallback decoupled from Custom sprite sheets toggle (2026-04-20): ore/seed rendering now always uses bundled `crystals_q90_1024x1024` tagged tiles by default, independent of the custom-sheets enablement and checklist, while still allowing enabled custom sheet tags to override crystal variants when available.
