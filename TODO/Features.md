- [ ] Combine multi-sheet custom map sprites by tags: load and mix tiles from all checked sprite sheets that have tagged JSON metadata, honor localStorage tag metadata overrides in singleplayer, force multiplayer to default sidecar metadata only, support rock overlay blending on biome ground like ore, and add a map-settings checked list (visible only when `Custom sprite sheets` is enabled, max 5 visible rows) to choose active sheets.
