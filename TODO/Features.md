...
- [ ] Expand integrated custom sprite-sheet runtime to compose tiles from all available `public/images/map/sprite_sheets/*.webp` + sidecar JSON tag files, apply per-sheet localStorage metadata overrides (single-player only), and expose per-sheet enable/disable checkboxes in Map Settings under `Custom sprite sheets` (scrollable to max 5 rows).
- [ ] Ensure integrated rock rendering supports blend-aware overlay on selected biome land tiles (same layered behavior as ore overlays) while remaining strictly cosmetic with no gameplay logic impact.
