...
- [x] SSE rectangular group tags + grouped tile placement (2026-04-19): users can select `group`, paint rectangular clusters that auto-tag as `group_X` with a sidebar `Group ID` input (1-999), release-to-increment group IDs, and runtime now prefers largest matching grouped tiles for rocks/decorative regions while debris uses building-footprint group matching (`WxH`) with `1x1` fallback.
