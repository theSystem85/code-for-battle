...
- [x] Street-tagged SSE rendering + default streets sheet (2026-04-23): integrated street tiles now default to `streets23_q90_1024x1024.webp` via static sheet selector/index defaults, pick biome-aware `street` candidates by valid adjacency tags (`top/bottom/left/right`) with best-match scoring, and skip SOT rendering on street host tiles.
