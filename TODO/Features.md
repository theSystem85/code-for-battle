- [x] SSE bitmask-direction tags follow-up (2026-04-23): generator-applied tags now split into directional tags per tile (`mask_t0/1`, `mask_r0/1`, `mask_b0/1`, `mask_l0/1`) instead of a single combined tag string.
- [x] SSE masks-tab overlap hotfix (2026-04-23): hardened masks-mode sidebar isolation by forcing non-mask sections to `display:none` (not only `hidden`) and preventing sheet metadata row re-show while Masks is active, plus always regenerating/loading the mask immediately when clicking the top `Masks` tab so the bitmask canvas appears instantly.
- [x] SSE masks-tab layout follow-up (2026-04-23): fixed top mode row to keep all sidebar-control tabs on one line (`Static/Animated/Masks`), ensured Masks mode actually switches the sidebar to masks-only controls (hides sprite-sheet selector/meta/converter + tagging inputs), kept tagging workflows in static/animated modes, and removed remaining overlap between masks inputs and tag controls.
- [x] SSE masks-mode follow-up (2026-04-23): moved `Masks` to the top mode tab row beside `Static/Animated`, made masks mode own the full sidebar (no mixed/overlapping controls), kept normal tagging UI available in static/animated modes for mask sheets, auto-applied bitmask debug labels as per-tile tags on generation, removed manual regenerate button (live update only), allowed fade distance `0`, and reduced redundant rotations (straight=2 entries, cross=1 entry).
- [x] SSE mask-generator follow-up (2026-04-23): moved autotile controls into a dedicated `Masks` sidebar tab, removed sidebar mini-preview and switched to immediate main-canvas regeneration on input change, rendered debug labels directly on the large canvas with tile click inspection, added debug-code info bubble, changed road fading to side-only behavior, grouped rotations by pattern columns (including T-junction column grouping), and added extra full-fill + wide-edge-fade columns for larger street compositions.
- [x] SSE deterministic 4-bit road autotile mask generator (2026-04-22): added integrated SSE generator module/UI for strict 16-pattern road masks with fixed bit order display (top/right/bottom/left = 1/2/4/8), clickable tile inspector, debug overlays, regenerate action, and PNG/WebP export with automated topology/dimension validation.
