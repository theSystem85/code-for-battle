- [x] SSE sidebar image converter panel (2026-04-18): added a bottom expand/collapse section with drag-drop local conversion for PNG/JPEG/WEBP to immediate WEBP download, configurable compression + width + height numeric inputs (defaults: 90, 1024x1024; max 2048x2048), and optional filename suffix toggle (`_{width}x{height}_q{compression}`).
