...
- [x] Add mobile portrait condensed guard-mode support: when friendly combat units are selected, show a Guard button to the left of Repair in the condensed action bar; tapping it toggles animated active state, allows selecting a friendly guard target, then auto-deactivates after guard assignment.
