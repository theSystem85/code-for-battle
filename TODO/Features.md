...
- [x] Street rendering GPU-path parity follow-up (2026-04-24): when GPU terrain base is active, CPU overlay pass now repaints street tiles through the streets23 tagged pipeline so street-sheet tiles appear even with `Custom sprite sheets` off, while keeping street-hosted SOT fully suppressed.
- [x] Street rendering follow-up hardening (2026-04-23): fully suppress street-hosted SOT in all render paths (including WebGL water-only overlays), always draw biome underlay beneath sprite-sheet street overlays (integrated biome when enabled, grass fallback when disabled), and keep tagged `streets23` street rendering active even when `Custom sprite sheets` is unchecked.
- [x] Street-sheet tag-driven rendering + street SOT suppression (2026-04-23): added bundled `streets23_q90_1024x1024.webp` to default SSE sheet lists/selectors, made street tile rendering resolve `street` + directional (`top`/`bottom`/`left`/`right`) tags from valid street neighbors with best-match scoring and biome-aware preference (`grass`/`soil`/`snow`/`sand`), and disabled SOT overlay rendering on street-hosted tiles.
