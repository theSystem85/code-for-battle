...
- [x] ctx.transform TypeError in shoreline mesh (2026-04-08): drawShorelineSegment used ctx.transform() matrix to set up local coordinate space for quad rendering, but this caused TypeError in some rendering contexts; replaced with direct polygon drawing (beginPath/lineTo/fill) using world-space createLinearGradient; removed unused getOrCreateShorelineTexture() method and unnecessary save/restore calls.
