...
## Bugs
- [x] Gentle ground-vs-environment collision refactor (2026-03-21): building/terrain/occupied-tile contact now reuses the same soft capped separation and normal damping profile as unit-vs-unit collisions, removing the old strong bounce-back environment response while keeping the resolution local and efficient; added unit regression coverage plus a Playwright scenario for live validation when browsers are available.
